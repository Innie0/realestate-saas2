  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="p-4 sm:p-6 text-white">
        ...
        {/* Create form */}
        {showCreate && (
          <form onSubmit={handleCreate} className="bg-[#111111] border border-white/10 rounded-xl p-5 mb-6 space-y-4">
            <div>
              <label className="block text-xs text-gray-400 mb-1.5">Property Address *</label>
        ...
        {loading ? (
          ...
          <div className="bg-[#111111] border border-white/10 rounded-xl p-10 text-center">
            ...
            <p className="text-gray-400 font-medium mb-1">No open houses yet</p>
          ...
          <div className="space-y-4">
            {openHouses.map(oh => {
              ...
              return (
                <div key={oh.id} className={`bg-[#111111] border rounded-xl p-5 ${isEnded ? 'border-white/5 opacity-60' : 'border-white/10'}`}>
                  <div className="flex flex-col sm:flex-row sm:items-start gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-2">
                        <MapPin className="w-4 h-4 text-gray-500 flex-shrink-0" />
                        <h3 className="font-semibold text-white text-sm truncate">{oh.property_address}</h3>
                        {isEnded && (
                          <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-white/5 text-gray-500">Ended</span>
                        )}
                      </div>
                      ...
                      <div className="flex items-center gap-2 mt-3">
                        <button
                          ...
                          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all border ${
                            isCopied ? 'bg-white/10 text-white border-white/20' : 'bg-white/5 text-gray-400 border-white/10 hover:text-white'
                          }`}
                        >
                        ...
                        <button
                          ...
                          className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium bg-white/5 text-gray-400 border border-white/10 hover:text-white transition-colors"
                        >
