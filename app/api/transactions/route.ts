...
import { resolveTransactionPartyLinks, isMissingSchemaFeatureError } from '@/lib/transaction-client-link';
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error && clientId && isMissingSchemaFeatureError(error)) {
      ...
      if (status && status !== 'all') {
        if (status === 'open') {
          legacyQuery.in('status', ['active', 'pending', 'under_contract']);
        } else {
          legacyQuery.eq('status', status);
        }
      }

      ({ data: transactions, error } = await legacyQuery);
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    let insertPayload = transactionData;
    ...
    if (error && isMissingSchemaFeatureError(error)) {
      const { buyer_client_id, seller_client_id, ...legacyPayload } = insertPayload;
