...
function SurfaceSkeleton({ lines = 3 }: { lines?: number }) {
  return (
    <div className="rounded-2xl bg-[var(--surface)] ring-1 ring-gray-900/[0.04] shadow-surface p-5 sm:p-6 animate-pulse space-y-3">
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="px-4 sm:px-6 py-6 sm:py-8 mx-auto w-full max-w-7xl space-y-5">
        {variant === 'account' && (
          <>
            <div className="rounded-2xl bg-[var(--surface)] ring-1 ring-gray-900/[0.04] shadow-surface p-5 sm:p-6 animate-pulse">
        ...
        {variant === 'list' && (
          <>
            <div className="flex flex-col sm:flex-row gap-3 animate-pulse">
              <div className="h-10 bg-[var(--surface)] border border-gray-200 rounded-xl flex-1" />
              <div className="h-10 w-full sm:w-32 bg-gray-100 rounded-xl" />
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {[1, 2, 3, 4, 5, 6].map((i) => (
                <div key={i} className="h-36 bg-[var(--surface)] border border-gray-200 rounded-2xl animate-pulse" />
              ))}
            </div>
          </>
        )}
        ...
        {variant === 'dashboard' && (
          <>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {[1, 2, 3].map((i) => (
                <div key={i} className="h-24 bg-[var(--surface)] rounded-2xl border border-gray-200 animate-pulse" />
              ))}
            </div>
            <div className="grid lg:grid-cols-3 gap-5">
              <div className="lg:col-span-2 h-64 bg-[var(--surface)] rounded-2xl border border-gray-200 animate-pulse" />
              <div className="h-64 bg-[var(--surface)] rounded-2xl border border-gray-200 animate-pulse" />
            </div>
          </>
        )}
        ...
        {variant === 'default' && (
          <>
            <div className="flex gap-1 p-1 bg-gray-100/80 rounded-xl animate-pulse max-w-md">
              <div className="flex-1 h-10 bg-[var(--surface)] rounded-lg" />
